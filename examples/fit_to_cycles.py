...
def loss_fxn(ff_params, cycle_batch):
    all_results = []
    indices = []
    preds = []
    for rfe, model in cycle_batch:
        indices.append(list(transform_index.get_transform_indices(rfe)))
    ...
    labels = jnp.asarray([rfe.label for rfe, _ in cycle_batch])
    loss = jnp.sum(jnp.abs(cycle_corrected_rbfes - labels))
...
if __name__ == "__main__":
    ...
    transform_index.build(relative_transformations)
    ...
    for steps in step_inds:
        ...
        (loss, (preds, stage_results)), loss_grads = jax.value_and_grad(loss_fxn, argnums=0, has_aux=True)(ordered_params, cycle_batch)

        results_this_step = {f"{stage}-{i}": result for i, (stage, result) in enumerate(stage_results)}

        print(f"at optimizer step {epoch}, loss={loss:.3f}")
        ...
        for stage, results in results_this_step.items():
            if _blew_up(results):
                blown_up = True
                print(f"step {epoch} blew up in {stage} stage")
        ...
        print(f'completed forcefield-updating step {epoch} in {elapsed:.3f} s !')

        # save du_dls snapshot
        path_to_du_dls = output_path.joinpath(f'du_dls_snapshot_{epoch}.npz')
        ...
        # results to npz
        path_to_npz = output_path.joinpath(f'theta_grad_loss_snapshot_{epoch}.npz')
        ...
        _save_forcefield(output_path.joinpath(f"forcefield_checkpoint_{epoch}.py"), ff_params=step_params)
