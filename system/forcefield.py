...
def parameterize(mol, forcefield, scale=1):
    """
    ...
    nrg_fns.append((
        ...
        (
            np.array(scale_matrix, dtype=np.float64),
    ...
    return nrg_fns, np.array(global_params, dtype=np.float64), np.array(global_param_groups, dtype=np.int32), conf, masses
