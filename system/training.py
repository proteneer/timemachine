...
data = open("/home/ubuntu/Relay/Code/timemachine/system/data_adam.csv","a+")
file = open("/home/ubuntu/Relay/Code/timemachine/system/output_adam.txt","a+")
plot = open("/home/ubuntu/Relay/Code/timemachine/system/plot_adam.csv","a+")
loss_plot = open("/home/ubuntu/Relay/Code/timemachine/system/loss_plot_adam.csv","a+")
...
sdf_file = open('/home/ubuntu/Relay/Code/benchmarksets/input_files/cd-set1/mol2/guest-1.mol2','r').read()
mol = Chem.MolFromMol2Block(sdf_file, sanitize=True, removeHs=False, cleanupSubstructures=True)
smirnoff = ForceField("test_forcefields/smirnoff99Frosst.offxml")
...
#differentiating with respect to charges
host_dp_idxs = np.argwhere(np.logical_or(host_param_groups == 7, host_param_groups == 4, host_param_groups == 5).reshape(-1)
guest_dp_idxs = np.argwhere(np.logical_or(smirnoff_param_groups == 7, smirnoff_param_groups == 4, smirnoff_param_groups == 5).reshape(-1)
combined_dp_idxs = np.argwhere(np.logical_or(combined_param_groups == 7, combined_param_groups == 4, combined_param_groups == 5).reshape(-1)
...
for t in range(35):
    ...
    for filename in files:
        ...
        sdf_file = open('/home/ubuntu/Relay/Code/benchmarksets/input_files/cd-set1/mol2/' + filename,'r').read()
        mol = Chem.MolFromMol2Block(sdf_file, sanitize=True, removeHs=False, cleanupSubstructures=True)
