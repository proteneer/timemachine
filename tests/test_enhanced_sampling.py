...
def test_gas_phase_importance_sampling():
    ...
    weighted_samples, log_weights = enhanced.generate_log_weighted_samples(
        mol, temperature, state.U_easy, state.U_decharged, seed
    )
...
def test_optimized_MTM():
    ...
    for _ in range(num_batches):
        ...
        ref_yvb, ref_prob, new_key = ref_mtm_mover.acceptance_probability(xvb_t, key)
        ...
        np.testing.assert_array_equal(new_key, test_key)
