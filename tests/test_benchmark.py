...
def generate_hif2a_frames(n_frames: int, frame_interval: int, seed=None, barostat_interval: int = 5, hmr: bool = True):
    st = get_hif2a_ligand_pair_single_topology("smirnoff_1_1_0_sc.py")

    # build the protein system.
    with resources.path("timemachine.testsystems.data", "hif2a_nowater_min.pdb") as path_to_pdb:
        host_system, host_coords, _, _, host_box, _ = builders.build_protein_system(str(path_to_pdb))

    initial_state = prepare_hif2a_initial_states(st, host_system, host_coords, host_box)
...
def prepare_hif2a_initial_states(st, host_system, host_coords, host_box):
    st = rbfe.SingleTopologyV3(st.mol_a, st.mol_b, st.core, st.ff)
...
def test_nonbonded_interaction_group_potential(hi2fa_test_frames):
    ...
    nonbonded_potential = pots[-1]
    # assert isinstance(nonbonded_potential, NonbondedInterpolated)
    num_param_batches = 5
