...
def _dhfr_system():
    ...
    host_fns, _, host_coords, box = setup_dhfr()

    nonbonded_bp = next(bp for bp in host_fns if isinstance(bp.potential, potentials.Nonbonded))
    host_conf = host_coords
    nonbonded_bp.potential.cutoff = 1.2
    return nonbonded_bp, host_conf, box


@pytest.fixture(scope="function")
def dhfr_system(_dhfr_system):
    nonbonded_bp, host_conf, box = _dhfr_system
    return nonbonded_bp, np.array(host_conf), np.array(box)


@pytest.mark.parametrize("precision", [np.float64, np.float32])
def test_nblist_hilbert(dhfr_system, precision):
    """
    ...
    nonbonded_bp, host_conf, box = dhfr_system
    nonbonded_fn = nonbonded_bp.potential
    N = host_conf.shape[0]
    ...
    xs = [host_conf, host_conf + deltas]

    # under pure fixed point accumulation the results should be identical.
    for x in xs:
        ref_du_dx, ref_du_dp, ref_u = ref_nonbonded_impl.execute(x, nonbonded_bp.params, box)
        test_du_dx, test_du_dp, test_u = test_nonbonded_impl.execute(x, nonbonded_bp.params, box)
        ...
        ref_du_dx = ref_nonbonded_impl.execute_du_dx(x, nonbonded_bp.params, box)
        test_du_dx = test_nonbonded_impl.execute_du_dx(x, nonbonded_bp.params, box)
...
def test_nblist_rebuild(dhfr_system, precision):
    """
...
def test_correctness(dhfr_system, select_atom_indices, num_atoms, precision, atol, rtol):
    """
    ...
    test_params = nonbonded_bp.params[:num_atoms, :]
    ...
    GradientTest().compare_forces(
        test_conf, test_params, box, potential, potential.to_gpu(precision), rtol=rtol, atol=atol
    )
