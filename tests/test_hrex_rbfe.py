...
def test_hrex_rbfe_hif2a(hif2a_single_topology_leg, seed, max_bisection_windows, target_overlap, enable_rest):
    ...
    if host_name == "complex" and md_params.water_sampling_params is not None:
        assert result.hrex_diagnostics.water_sampling_proposals_by_state_by_iter.shape == (
            md_params.n_frames,
            final_windows,
            2,
        )
        ...
        assert np.all(result.hrex_diagnostics.water_sampling_proposals_by_state_by_iter[:, :, 1] == proposals_per_frame)
        assert np.all(result.hrex_diagnostics.water_sampling_proposals_by_state_by_iter[:, :, 0] >= 0)
        assert result.hrex_diagnostics.cumulative_water_sampling_proposals.shape == (final_windows, 2)
    else:
        assert result.hrex_diagnostics.water_sampling_proposals_by_state_by_iter is None
