    ...
    def test_optimize_ensemble(self):
        ...
        with tf.variable_scope("bad"):
            bad_intg = integrator.LangevinIntegrator(
                masses, x_ph, [hb, ha], dt, friction, temp, disable_noise=False, buffer_size=3000)
        ...
        # keep everything the same except time-step
        sess.run([
            # tf.assign(bond_params[0], 99.96298709576533),
            # tf.assign(bond_params[1], 1.0249079216520398),
            # tf.assign(angle_params[0], 75.02126373303723),
            # tf.assign(angle_params[1], 1.758724601901881),
