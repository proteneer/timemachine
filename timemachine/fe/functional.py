...
def _make_selection_mask(compute_du_dx=False, compute_du_dp=False, compute_u=False):
    return compute_du_dx, compute_du_dp, compute_u
...
def wrap_impl(impl, pack=lambda x: x):
    ...
    @custom_jvp
    def U(coords, params, box) -> float:
        selection = _make_selection_mask(compute_u=True)
        result_tuple = impl.execute_selective(coords, pack(params), box, *selection)
    ...
    def U_jvp(primals, tangents) -> Tuple[float, float]:
        ...
        # unpack inputs
        coords, _params, box = primals
        coords_dot, _params_dot, box_dot = tangents
        ...
        # call custom op once
        result_tuple = impl.execute_selective(coords, params, box, *selection)

        # unpack result tuple
        primal_out = result_tuple[2]
        coords_grad, params_grad = result_tuple[:2]
...
def construct_differentiable_interface(unbound_potentials: Sequence[Potential], precision=jnp.float32):
    ...
    >>> _ = grad(U, (0,1,3))(coords, sys_params, box)
    ...
    def U(coords, params, box):
        return jnp.sum(jnp.array([U_i(coords, p_i, box) for (U_i, p_i) in zip(U_s, params)]))
...
def construct_differentiable_interface_fast(unbound_potentials: Sequence[Potential], params, precision=jnp.float32):
    ...
    >>> _ = grad(U)(coords, sys_params, box)
