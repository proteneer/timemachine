...
def GBSAReference(*args, precision):
    dim = args[-1]
    if precision == np.float64:
        if dim == 3:
            return custom_ops.GBSAReference_f64_3d(*args[:-1])
        elif dim == 4:
            return custom_ops.GBSAReference_f64_4d(*args[:-1])
        else:
            raise Exception("Bad Dim")
    elif precision == np.float32:
        if dim == 3:
            return custom_ops.GBSAReference_f32_3d(*args[:-1])
        elif dim == 4:
            return custom_ops.GBSAReference_f32_4d(*args[:-1])
        else:
            raise Exception("Bad Dim")
    else:
        raise Exception("Unknown precision")
