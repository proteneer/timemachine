...
class CustomOpsTest(unittest.TestCase):
    ...
    def assert_derivatives(self, conf, params, ref_nrg, test_nrg, compare_mixed_partials=True):
        ...
        for idx, dp_idxs in enumerate(all_dp_idxs):
            ...
            test_d2e_dx2 = np.transpose(test_d2e_dx2, [0,2,1,4,3]) # C NDND -> C DNDN
            # print(test_d2e_dx2)
            test_d2e_dxdp = np.transpose(test_d2e_dxdp, [0,1,3,2])

            ref_e, ref_de_dx, ref_d2e_dx2, ref_de_dp, ref_d2e_dxdp = generate_derivatives(ref_nrg, confs, params)

            # important we transpose the ref to deal with symmetry issues
            # print("BEFORE SHAPE", ref_d2e_dx2.shape)
            # ref_d2e_dx2 = np.transpose(ref_d2e_dx2, [0,2,1,4,3])
            # print("AFTER SHAPE", ref_d2e_dx2.shape)
            # print("CMOP SHAPE", test_d2e_dx2.shape)
            ...
            # batch compare first order terms
            # np.testing.assert_almost_equal(test_de_dp, ref_de_dp[:, dp_idxs])
            # print("?", test_d2e_dxdp)
            # print("!", ref_d2e_dxdp[:, dp_idxs, :, :])

            if compare_mixed_partials:
            ...
            for index, (t, r) in enumerate(zip(test_d2e_dx2, ref_d2e_dx2)):
                print("Hessians testing:", index)

                print(">>>>>>>>>>", t.shape, r.shape)
                t = symmetrize(t)
                np.testing.assert_almost_equal(t, r)

            # np.testing.assert_allclose(ref_d2e_dxdp[:, dp_idxs, :, :], test_d2e_dxdp, rtol=1e-12)

                # buggy if one of the array has 0
    ...
    def assert_derivatives_mixed_dimensions(self, conf4d, params, ref_nrg, test_nrg):
        ...
        for idx, dp_idxs in enumerate(all_dp_idxs):
            ...
            for t, r in zip(test_d2e_dx2, ref_d2e_dx2):
                ...
                # print(test_tril, ref_tril)
                np.testing.assert_almost_equal(ref_tril, test_tril)
                np.testing.assert_allclose(ref_tril, test_tril, rtol=1e-12)

            # batch compare
            np.testing.assert_almost_equal(test_de_dp, ref_de_dp[:, dp_idxs])
            # [C, P, N, 3]
            np.testing.assert_almost_equal(test_d2e_dxdp[:, :, :, :3], ref_d2e_dxdp[:, dp_idxs, :, :])
            np.testing.assert_allclose(ref_d2e_dxdp[:, dp_idxs, :, :], test_d2e_dxdp[:, :, :, :3], rtol=1e-12)
...
class TestLennardJones(CustomOpsTest):
    ...
    # def test_derivatives_large(self):

    #     num_atoms = 36

    #     np.random.seed(123)
    #     x0 = np.array(np.random.rand(num_atoms, 3)*4, dtype=np.float64)
    #     params = np.array([3.0, 2.0, 1.0, 1.4], dtype=np.float64)/10
    #     param_idxs = np.random.randint(0, 4, size=(num_atoms, 2)).astype(np.int32)
    #     scale_matrix = 1-np.eye(num_atoms, dtype=np.int32)
    ...
    #     gather_idxs = np.arange(x0.shape[0], dtype=np.int32)
    #     np.random.shuffle(gather_idxs)
    ...
    # def test_derivatives_zero_eps(self):
    ...
    #     #                   e0   e1   s0
    #     params = np.array([0.0, 0.1, 1.0], dtype=np.float64)
    #     param_idxs = np.array([
    #         [2, 0],
    #         [2, 0],
    #         [2, 1]], dtype=np.int32)
    ...
    def test_derivatives(self):
        ...
        energy_fn_cutoff = functools.partial(nonbonded.lennard_jones,
            scale_matrix=scale_matrix,
            param_idxs=param_idxs,
            box=None,
            cutoff=cutoff)

        lj_cutoff = custom_ops.LennardJones_f64(
            scale_matrix,
            gather_idxs,
            param_idxs,
            cutoff
...
#         gather_idxs = np.arange(conf.shape[0], dtype=np.int32)
...
#         energy_fn_cutoff = functools.partial(
#             nonbonded.electrostatics,
#             param_idxs=param_idxs,
#             scale_matrix=scale_matrix,
#             box=None,
...
#         es_cutoff = custom_ops.Electrostatics_f64(
#             scale_matrix,
#             gather_idxs,
#             param_idxs,
#             2.0
...
#         np.random.seed(123)
#         x0 = np.array(np.random.rand(num_atoms, 3)*2, dtype=np.float64)
#         params = np.array([3.0, -2.0, 1.0, -1.4], dtype=np.float64)/4
#         param_idxs = np.random.randint(0, 4, size=(num_atoms)).astype(np.int32)
#         scale_matrix = 1-np.eye(num_atoms, dtype=np.int32)

#         scale_matrix = np.zeros_like(scale_matrix)

#         # diagonals should be zero as well
#         # scale_matrix[0, :] = 1
#         # scale_matrix[:, 0] = 1
#         # scale_matrix[num_atoms-1, :] = 1
#         # scale_matrix[:, num_atoms-1] = 1

#         # scale_matrix[0][0] = 0
#         # scale_matrix[num_atoms-1][num_atoms-1] = 0

#         cutoff = 2.0

#         energy_fn = functools.partial(nonbonded.electrostatics,
#             scale_matrix=scale_matrix,
#             param_idxs=param_idxs,
#             box=None,
#             cutoff=cutoff)
