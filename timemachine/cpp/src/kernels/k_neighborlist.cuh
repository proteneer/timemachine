...
void __global__ k_find_block_bounds(
    ...
    const unsigned int *__restrict__ idxs, // [num_indices]
    const double *__restrict__ coords,     // [N*3]
    const double *__restrict__ box,        // [3*3]
    double *__restrict__ block_bounds_ctr, // [T*3]
    double *__restrict__ block_bounds_ext  // [T*3]
) {
    ...
    const int last = min(base + tile_size, num_indices);
...
void __global__ k_find_blocks_with_ixns(
    ...
    const double *__restrict__ column_bb_ext,     // [N * 3] block extants
    ...
    // Only possible when performing upper triangle, as otherwise indices may not be ordered
    // and will cause the kerenl to hang
    const bool single_periodic_box = UPPER_TRIAG && (static_cast<RealType>(0.5) * bx - row_bb_ext_x >= cutoff &&
