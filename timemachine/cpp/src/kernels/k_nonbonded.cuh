...
void __global__ k_compute_nonbonded_target_atom_energies(
    ...
    const int atom_i_idx = target_atoms[row_idx];
    const int mol_i_idx = target_mols[row_idx];

    const int min_mol_offset = target_mols_offsets[mol_i_idx];
    const int max_mol_offset = target_mols_offsets[mol_i_idx + 1];
    const int min_atom_idx = target_atoms[min_mol_offset];
    const int max_atom_idx = target_atoms[max_mol_offset - 1];
