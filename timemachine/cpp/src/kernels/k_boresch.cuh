...
void __global__ k_boresch_bond_inference(
    ...
    for(int d=0; d < 3; d++) {
        RealType grad_delta = lambda_full*kb*db*dx[d]/dij;
    ...
    atomicAdd(energy, lambda_full*kb/2*db*db);
    atomicAdd(du_dl, lambda_flag*kb/2*db*db);
...
void __global__ k_boresch_angle_inference(
    ...
    for(int d=0; d < MAXDIM; d++) {
        RealType grad_i = lambda_full*ka*delta*(rij[d]*top/(n3ij*njk) + (-rjk[d])/nijk);
        atomicAdd(grad_coords + i_idx*D + d, static_cast<unsigned long long>((long long) (grad_i*FIXED_EXPONENT)));

        RealType grad_j = lambda_full*ka*delta*((-rij[d]*top/(n3ij*njk) + (-rjk[d])*top/(nij*n3jk) + (rij[d] + rjk[d])/nijk));
        atomicAdd(grad_coords + j_idx*D + d, static_cast<unsigned long long>((long long) (grad_j*FIXED_EXPONENT)));

        RealType grad_k = lambda_full*ka*delta*(-rij[d]/nijk + rjk[d]*top/(nij*n3jk));
        atomicAdd(grad_coords + k_idx*D + d, static_cast<unsigned long long>((long long) (grad_k*FIXED_EXPONENT)));
    }

    atomicAdd(out_energy, lambda_full*ka/2*delta*delta);
    atomicAdd(out_du_dl, lambda_flag*ka/2*delta*delta);
