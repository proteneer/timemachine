...
void __global__ k_convert_energies_to_log_weights(
    const int N, const RealType inv_beta, const __int128 *__restrict__ energies, RealType *__restrict__ weights) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    while (idx < N) {
        __int128 energy = energies[idx];
        weights[idx] = fixed_point_overflow(energy) ? INFINITY : inv_beta * FIXED_TO_FLOAT<RealType>(energy);
        idx += gridDim.x * blockDim.x;
    }
}

template void __global__ k_convert_energies_to_log_weights<float>(
    const int N, const float inv_beta, const __int128 *__restrict__ energies, float *__restrict__ weights);
template void __global__ k_convert_energies_to_log_weights<double>(
    const int N, const double inv_beta, const __int128 *__restrict__ energies, double *__restrict__ weights);
