...
void declare_potential_executor(py::module &m) {
    ...
    py::class_<Class, std::shared_ptr<Class>>(m, pyclass_name.c_str(), py::buffer_protocol(), py::dynamic_attr())
        ...
        .def("num_atoms", &timemachine::PotentialExecutor::num_atoms)
        .def(
            "execute_bound",
            [](timemachine::PotentialExecutor &runner,
               ...
               bool compute_u) -> py::tuple {
                ...
                runner.execute_potentials(
                    ...
                    param_ptrs,
                    compute_du_dx ? &du_dx[0] : nullptr,
                    nullptr, // Don't compute du_dp for bound pots
                    compute_u ? &u[0] : nullptr);
        ...
        .def(
            ...
            [](timemachine::PotentialExecutor &runner,
               ...
               const bool compute_u) -> py::tuple {
                ...
                std::vector<double *> param_ptrs;
                int offset = 0;
                for (auto size : vec_param_sizes) {
                    param_ptrs.push_back(param_buffer.data + offset);
                    offset += size;
                }
