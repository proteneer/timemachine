...
SmoothcoreNonbondedInteractionGroup<RealType>::SmoothcoreNonbondedInteractionGroup(
    const int N,
    const std::vector<int> &row_atom_idxs,
    const std::vector<int> &col_atom_idxs,
    const std::vector<int> &anchor_atom_idxs,
    const double beta,
    const double cutoff,
    ...
    gpuErrchk(cudaMemcpy(d_anchor_idxs_, &anchor_atom_idxs[0], N_ * sizeof(*d_anchor_idxs_), cudaMemcpyHostToDevice));
...
template <typename RealType> SmoothcoreNonbondedInteractionGroup<RealType>::~SmoothcoreNonbondedInteractionGroup() {
    gpuErrchk(cudaFree(d_col_atom_idxs_));
    gpuErrchk(cudaFree(d_row_atom_idxs_));
    gpuErrchk(cudaFree(d_anchor_idxs_));

    gpuErrchk(cudaFree(d_perm_));
    gpuErrchk(cudaFree(d_rescaled_x_));
...
template <typename RealType> bool SmoothcoreNonbondedInteractionGroup<RealType>::needs_sort() {
    return steps_since_last_sort_ % STEPS_PER_SORT == 0;
}

template <typename RealType>
void SmoothcoreNonbondedInteractionGroup<RealType>::sort(
...
template <typename RealType>
void SmoothcoreNonbondedInteractionGroup<RealType>::execute_device(
    const int N,
    const int P,
    const double *d_x,
    ...
    if (N != N_) {
        throw std::runtime_error(
            "SmoothcoreNonbondedInteractionGroup::execute_device(): expected N == N_, got N=" + std::to_string(N) +
            ", N_=" + std::to_string(N_));
    }

    if (P != N_ * PARAMS_PER_ATOM) {
        throw std::runtime_error(
            "SmoothcoreNonbondedInteractionGroup::execute_device(): expected P == N_*" +
            std::to_string(PARAMS_PER_ATOM) + ", got P=" + std::to_string(P) + ", N_*" +
            std::to_string(PARAMS_PER_ATOM) + "=" + std::to_string(N_ * PARAMS_PER_ATOM));
    }
    ...
    // initialize rescaled coordinates
    gpuErrchk(cudaMemcpyAsync(d_rescaled_x_, d_x, N_ * 3 * sizeof(*d_rescaled_x_), cudaMemcpyDeviceToDevice, stream));

    const int tpb = DEFAULT_THREADS_PER_BLOCK;
    const int B = ceil_divide(N_, tpb);

    k_smoothcore_rescale_coords<RealType, 3, PARAMS_PER_ATOM>
        <<<B, tpb, 0, stream>>>(N_, nblist_.get_row_idxs(), d_anchor_idxs_, d_x, d_p, d_rescaled_x_);
    ...
    if (this->needs_sort()) {
        ...
        this->sort(d_rescaled_x_, d_box, stream);
    } else {
    ...
    k_smoothcore_gather_coords_and_params_zero_w<double, 3, PARAMS_PER_ATOM>
        <<<ceil_divide(K, tpb), tpb, 0, stream>>>(K, d_perm_, d_rescaled_x_, d_p, d_sorted_x_, d_sorted_p_);
    gpuErrchk(cudaPeekAtLastError());
    ...
    if (p_rebuild_nblist_[0] > 0) {
        ...
        gpuErrchk(cudaMemcpyAsync(
            d_nblist_x_, d_rescaled_x_, N * 3 * sizeof(*d_rescaled_x_), cudaMemcpyDeviceToDevice, stream));
    ...
    kernel_idx |= (d_du_dx || d_du_dp) ? 1 << 1 : 0; // we need forces if we request d_du_dp
    kernel_idx |= d_u ? 1 << 2 : 0;

    // note that these kernels will no longer trigger the "ALCHEMICAL=1" template option since we've set
    // the w-params to be strictly 0.
    ...
    // coords are (N,3)
    if (d_du_dx) {
        k_smoothcore_rescale_and_accum_du_dx<RealType, 3, PARAMS_PER_ATOM>
            <<<B, tpb, 0, stream>>>(N_, d_anchor_idxs_, d_x, d_p, d_perm_, d_sorted_du_dx_, d_du_dx);
        gpuErrchk(cudaPeekAtLastError());
    }

    // params are (N, PARAMS_PER_ATOM)
    if (d_du_dp) {
        k_smoothcore_rescale_and_accum_du_dp<RealType, 3, PARAMS_PER_ATOM>
            <<<B, tpb, 0, stream>>>(N_, d_anchor_idxs_, d_x, d_p, d_perm_, d_sorted_du_dp_, d_sorted_du_dx_, d_du_dp);
        gpuErrchk(cudaPeekAtLastError());
    }
    if (d_u) {
...
template <typename RealType>
void SmoothcoreNonbondedInteractionGroup<RealType>::set_atom_idxs(
...
template <typename RealType>
void SmoothcoreNonbondedInteractionGroup<RealType>::set_atom_idxs_device(
    const int NC,
    const int NR,
...
template <typename RealType>
void SmoothcoreNonbondedInteractionGroup<RealType>::du_dp_fixed_to_float(
...
template <typename RealType>
void SmoothcoreNonbondedInteractionGroup<RealType>::validate_idxs(
