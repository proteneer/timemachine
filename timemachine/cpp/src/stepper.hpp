...
public:
    ...
    virtual void forward_step(const int N, const double *coords, unsigned long long *dx) = 0;
...
public:
    ...
    AlchemicalStepper(std::vector<Gradient *> forces, const std::vector<double> &lambda_schedule);

    virtual void forward_step(const int N, const double *coords, unsigned long long *dx) override;
    ...
    int get_T() const { return lambda_schedule_.size(); }

    int get_F() const { return forces_.size(); }

    void set_du_dl_adjoint(const int FT, const double *adj);
};
