...
BDExchangeMove<RealType>::BDExchangeMove(
    ...
    : N_(N), num_target_mols_(target_mols.size()), beta_(static_cast<RealType>(1.0 / (BOLTZ * temperature))),
      mol_potential_(N, target_mols, nb_beta, cutoff), sampler_(N, seed), logsumexp_(N), d_intermediate_coords_(N * 3),
      d_params_(params.size()), d_mol_energy_buffer_(num_target_mols_),
...
void BDExchangeMove<RealType>::move_device(
    ...
    gpuErrchk(cudaMemcpyAsync(
        d_intermediate_coords_.data, d_coords, N * sizeof(*d_coords), cudaMemcpyDeviceToDevice, stream));
    ...
    for (int move = 0; move < num_moves; move++) {
        ...
        sampler_.sample_device(N, num_samples, d_log_weights_.data, d_samples_.data, stream);
