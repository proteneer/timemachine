...
std::array<std::vector<double>, 2> Context::multiple_steps_local(
    ...
    try {
        ...
        const auto d_free_indices = local_md_config_->get_free_indices();

        std::vector<std::shared_ptr<BoundPotential>> local_pots = local_md_config_->get_potentials();

        intg_->initialize(local_pots, d_x_t_, d_v_t_, d_box_t_, d_free_indices->data, stream);
        for (int i = 0; i < burn_in; i++) {
            this->_step(local_pots, d_free_indices->data, stream);
        }
        ...
        intg_->finalize(local_pots, d_x_t_, d_v_t_, d_box_t_, d_free_indices->data, stream);
