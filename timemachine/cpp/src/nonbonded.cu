...
void Nonbonded<RealType, Interpolated>::du_dp_fixed_to_float(
    ...
    for (int i = 0; i < num_tuples; i++) {
        {
            const int idx = i * 3 + 0;
            du_dp_float[idx] = FIXED_TO_FLOAT_DU_DP<double, FIXED_EXPONENT_DU_DCHARGE>(du_dp[idx]);
        }
        {
            const int idx = i * 3 + 1;
            du_dp_float[idx] = FIXED_TO_FLOAT_DU_DP<double, FIXED_EXPONENT_DU_DSIG>(du_dp[idx]);
        }
        {
            const int idx = i * 3 + 2;
            du_dp_float[idx] = FIXED_TO_FLOAT_DU_DP<double, FIXED_EXPONENT_DU_DEPS>(du_dp[idx]);
        }
    }
}
