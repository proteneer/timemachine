...
void PotentialExecutor::execute_potentials(
    ...
    const double *h_x,
    const double *h_box,
    const std::vector<double *> &d_params, // Vector of pointers to device arrays
    ...
    if (h_du_dp != nullptr) {
        du_dp_buffer.reset(new DeviceBuffer<unsigned long long>(num_params));
    ...
    for (int i = 0; i < pots.size(); i++) {
        ...
        int pot_param_count = param_sizes[i];
        pots[i]->execute_device(
            N_,
            pot_param_count,
            d_x_.data,
            pot_param_count > 0 ? d_params[i] : nullptr,
            d_box_.data,
