...
class CustomOpsTest(unittest.TestCase):
    ...
    def assert_derivatives(self, conf, params, ref_nrg, test_nrg):
        ...
        for dp_idxs in all_dp_idxs:
            ...
            debug0 = np.tril(np.reshape(test_d2e_dx2, (num_atoms*3, num_atoms*3)))
            debug1 = np.tril(np.reshape(ref_d2e_dx2, (num_atoms*3, num_atoms*3)))
            np.testing.assert_almost_equal(debug0, debug1)
            # np.testing.assert_almost_equal(np.triu(test_d2e_dx2), np.triu(ref_d2e_dx2))
