...
nvcc -std=c++11 --ptxas-options=-v -lineinfo -O2 -arch=sm_70 -Xcompiler -fPIC -I gpu/ gpu/custom_bonded_gpu.cu gpu/custom_nonbonded_gpu.cu gpu/potential.cu optimizers/optimizer.cu optimizers/langevin.cu optimizers/gpu_utils.cu -c
...
g++ -O3 -march=native -Wall -shared -std=c++11 -fPIC $PLATFORM_FLAGS `python3 -m pybind11 --includes` -L/usr/local/cuda/lib64/ -I/usr/local/cuda/include/ wrap_kernels.cpp custom_bonded_gpu.o custom_nonbonded_gpu.o langevin.o optimizer.o potential.o gpu_utils.o -o custom_ops`python3-config --extension-suffix` -lcurand -lcublas -lcudart
