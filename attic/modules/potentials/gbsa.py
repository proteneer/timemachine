# taken from josh fass's code but fixed a couple of bugs
# https://github.com/openforcefield/bayes-implicit-solvent/blob/propertycalculator/bayes_implicit_solvent/gb_models/jax_gb_models.py

import jax.numpy as jnp

from timemachine.potentials.jax_utils import convert_to_4d, distance


def step(x):
    # return (x > 0)
    return 1.0 * (x >= 0)


def gbsa_obc(
    coords,
    # params,
    lamb,
    # box,
    charge_params,
    gb_params,
    # charge_idxs,
    # radii_idxs,
    # scale_idxs,
    alpha,
    beta,
    gamma,
    cutoff_radii,
    cutoff_force,
    lambda_plane_idxs,
    lambda_offset_idxs,
    dielectric_offset=0.009,
    surface_tension=28.3919551,
    solute_dielectric=1.0,
    solvent_dielectric=78.5,
    probe_radius=0.14,
):

    box = None

    assert cutoff_radii == cutoff_force

    coords_4d = convert_to_4d(coords, lamb, lambda_plane_idxs, lambda_offset_idxs, cutoff_radii)

    N = len(charge_params)

    radii = gb_params[:, 0]
    scales = gb_params[:, 1]

    ri = jnp.expand_dims(coords_4d, 0)
    rj = jnp.expand_dims(coords_4d, 1)

    dij = distance(ri, rj, box)

    eye = jnp.eye(N, dtype=dij.dtype)

    r = dij + eye  # so I don't have divide-by-zero nonsense
    or1 = radii.reshape((N, 1)) - dielectric_offset
    or2 = radii.reshape((1, N)) - dielectric_offset
    sr2 = scales.reshape((1, N)) * or2

    L = jnp.maximum(or1, abs(r - sr2))
    U = r + sr2

    I = 1 / L - 1 / U + 0.25 * (r - sr2 ** 2 / r) * (1 / (U ** 2) - 1 / (L ** 2)) + 0.5 * jnp.log(L / U) / r
    # handle the interior case
    I = jnp.where(or1 < (sr2 - r), I + 2 * (1 / or1 - 1 / L), I)
    I = step(r + sr2 - or1) * 0.5 * I  # note the extra 0.5 here
    I -= jnp.diag(jnp.diag(I))

    # switch I only for now
    # inner = (np.pi*np.power(dij,8))/(2*cutoff_radii)
    # sw = np.power(np.cos(inner), 2)
    # I = I*sw

    I = jnp.where(dij > cutoff_radii, 0, I)
    I = jnp.sum(I, axis=1)

    # okay, next compute born radii
    offset_radius = radii - dielectric_offset

    psi = I * offset_radius

    psi_coefficient = alpha
    psi2_coefficient = beta
    psi3_coefficient = gamma

    psi_term = (psi_coefficient * psi) - (psi2_coefficient * psi ** 2) + (psi3_coefficient * psi ** 3)

    B = 1 / (1 / offset_radius - jnp.tanh(psi_term) / radii)

    E = 0.0
    # single particle
    # ACE
    E += jnp.sum(surface_tension * (radii + probe_radius) ** 2 * (radii / B) ** 6)

    # on-diagonal
    charges = charge_params

    E += jnp.sum(-0.5 * (1 / solute_dielectric - 1 / solvent_dielectric) * charges ** 2 / B)

    # particle pair
    f = jnp.sqrt(r ** 2 + jnp.outer(B, B) * jnp.exp(-(r ** 2) / (4 * jnp.outer(B, B))))
    charge_products = jnp.outer(charges, charges)

    ixns = -(1 / solute_dielectric - 1 / solvent_dielectric) * charge_products / f

    # sw = np.power(np.cos((np.pi*dij)/(2*cutoff_radii)), 2)
    # ixns = ixns*sw
    ixns = jnp.where(dij > cutoff_force, 0, ixns)

    E += jnp.sum(jnp.triu(ixns, k=1))

    return E
