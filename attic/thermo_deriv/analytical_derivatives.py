...
Z_integrand = functools.partial(
    pdf,
    observable_fn=lambda conf, params: 1.,
    kT=kT
)
...
avg_x_integrand = functools.partial(
    pdf,
    observable_fn=lambda conf, params: conf[1][0],
    kT=BOLTZ*temperature
)
...
avg_x_dot_du_dp_integrand = functools.partial(
    pdf,
    observable_fn=lambda conf, params: conf[1][0]*dU_dp(conf, params)[0],
    kT=BOLTZ*temperature
)
